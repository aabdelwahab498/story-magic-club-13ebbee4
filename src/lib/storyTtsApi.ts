...
// Adapter boundary — backed by Supabase edge functions until Backend Core
// media endpoints are live.
...
export const useGenerateFullNarration = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["my_ai_stories"] });
      qc.invalidateQueries({ queryKey: ["my_ai_stories_page"] });
