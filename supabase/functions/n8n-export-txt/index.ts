// n8n-export-txt (renamed pipeline; keeps name for compatibility)
// Fully in-app TXT export — no external workflows. Renders a BOM-safe UTF-8
// story file from the client payload, uploads it to `story-exports`, returns
// a signed URL.
...
function safeSlug(input: string, fb = "story") {
  const c = (input || "").replace(/[^\p{L}\p{N}\-_ ]+/gu, "").replace(/\s+/g, "-").slice(0, 60);
  return c || fb;
}
...
Deno.serve(async (req) => {
  ...
  const rl = await checkRateLimit(`u:${userId}`, "export-txt", { windowSec: 3600, max: 10, blockSec: 300 });
  ...
  const { data: exportRow, error: insertErr } = await admin.from("exports").insert({
    ...
    status: "generating",
    metadata: {
      title: payload.title, child_name: payload.child_name ?? null,
      emotion_tags: payload.emotion_tags ?? [], page_count: payload.page_count ?? null,
    },
  }).select("id").single();
  if (insertErr || !exportRow) return friendly("db_insert_failed", 500);
  const exportId = exportRow.id as string;

  // Compose file: BOM + title + child + language header + body.
  const header = [
    payload.title,
  ...
  await admin.from("exports").update({
    ...
    file_size: bytes.byteLength, provider: "local", expires_at: expiresAt,
  }).eq("id", exportId);
